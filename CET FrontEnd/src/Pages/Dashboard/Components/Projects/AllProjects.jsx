import { BookOpen, SquarePen, Trash2 } from 'lucide-react';
import React from 'react';
import { Link } from 'react-router';

const AllProjects = ({ showDetails, setShowDetails }) => {



    const totalTask = 20;
    const doneTask = 12;

    const progress = (doneTask / totalTask) * 100;

    return (
        <div className='max-w-[1330px]'>

            <div className="w-full mt-5 flex flex-col">
                {/* <div className="flex gap-5 font-rubik bg-[rgba(229,231,235,0.25)] p-4 border-b border-[#e5e7eb] *:font-semibold">
                    <div className="w-[5%]">
                        SL
                    </div>
                    <div className="w-[20%]">
                        Project
                    </div>
                    <div className="w-[20%]">
                        Priority
                    </div>
                    <div className="w-[20%]">
                        Deadline
                    </div>
                    <div className="w-[20%]">
                        Assigned To
                    </div>
                    <div className="w-[20%]">
                        Functions
                    </div>

                </div> */}





                {/* <div className="flex text-sm justify-center items-center gap-5 font-rubik bg-white p-4 border-b border-[#e5e7eb] *:font-semibold"
                    onClick={() => setShowDetails(true)}
                >
                    <div className="w-[5%]">
                        # 1
                    </div>
                    <div className="w-[20%] text-left">
                        SSl BD Web Application
                    </div>
                    <div className="w-[20%] flex justify-center items-center">
                        <div className=" border flex justify-center items-center border-amber-300 rounded-2xl pl-1 py-1 pr-2">
                            <p className='h-4 w-4 rounded-4xl bg-amber-300 mr-1'></p>
                            <p>Priority</p>
                        </div>
                    </div>
                    <div className="w-[20%]">
                        11/12/2026 - 1:00 PM
                    </div>
                    <div className="w-[20%] flex gap-2 justify-center items-center">
                        <div className="flex">
                            <div className="w-7  h-7  rounded-4xl bg-amber-200"></div>
                            <div className="w-7  h-7 -ml-3  rounded-4xl bg-amber-500"></div>
                            <div className="w-7  h-7 -ml-3 rounded-4xl bg-amber-200"></div>
                        </div>
                        <div className="flex cursor-pointer">
                            <p>+2 More</p>
                        </div>

                    </div>
                    <div className="w-[20%] flex gap-1">
                        <div className="flex justify-center items-center border border-[#e5e7eb] p-1.5 gap-1 cursor-pointer">
                            <BookOpen size={16} /> <p>View</p>
                        </div>
                        <div className="flex justify-center items-center border border-[#e5e7eb] p-1.5 gap-1 cursor-pointer">
                            <SquarePen size={16} /> <p>Update</p>
                        </div>
                        <div className="flex justify-center items-center border border-[#e5e7eb] p-1.5 gap-1 cursor-pointer">
                            <Trash2 size={16} /> <p>Delete</p>
                        </div>
                    </div>

                </div> */}


                <div className="grid grid-cols-3 gap-5">
                    <Link to="/Dashboard/Projects/ProjectName"
                        // onClick={() => setShowDetails(true)}
                        className="group text-left relative bg-white border border-[#e5e7eb] rounded-2xl p-5 shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer"
                    >
                        {/* Header */}
                        <div className="flex justify-between items-start">

                            {/* Title Section */}
                            <div>
                                <p className="text-xs text-gray-400 font-semibold">#1 PROJECT</p>
                                <h3 className="text-lg font-semibold text-gray-800">
                                    SSl BD Web Application
                                </h3>
                            </div>

                            {/* Priority Badge */}
                            <div className="flex items-center border border-amber-300 bg-amber-50 text-amber-600 rounded-full px-3 py-1 text-xs font-semibold">
                                <div className="w-2 h-2 bg-amber-400 rounded-full mr-2"></div>
                                Priority
                            </div>

                        </div>

                        {/* Date */}
                        <p className="text-sm text-gray-500 mt-2">
                            Due: 11/12/2026 - 1:00 PM
                        </p>

                        {/* Progress Section */}
                        <div className="mt-4">

                            <div className="flex justify-between text-xs font-semibold text-gray-500 mb-1">
                                <p>Progress</p>
                                <p>{doneTask}/{totalTask} Tasks</p>
                            </div>

                            <div className="w-full bg-gray-200 h-2 rounded-full overflow-hidden">
                                <div
                                    style={{ width: `${progress}%` }}
                                    className="h-full bg-emerald-500 rounded-full transition-all duration-500"
                                ></div>
                            </div>

                        </div>

                        {/* Bottom Section */}
                        <div className="flex justify-between items-center mt-5">

                            {/* Team */}
                            <div className="flex items-center">

                                <div className="flex">
                                    <div className="w-8 h-8 rounded-full bg-amber-200 border-2 border-white"></div>
                                    <div className="w-8 h-8 -ml-3 rounded-full bg-amber-500 border-2 border-white"></div>
                                    <div className="w-8 h-8 -ml-3 rounded-full bg-amber-200 border-2 border-white"></div>
                                </div>

                                <p className="text-sm text-gray-500 ml-2">+2 more</p>

                            </div>

                            {/* Actions */}
                            <div className="flex gap-2">

                                <button className="flex items-center gap-1 text-xs border border-gray-200 px-2 py-1 rounded-lg hover:bg-gray-100">
                                    <BookOpen size={14} /> View
                                </button>

                                <button className="flex items-center gap-1 text-xs border border-gray-200 px-2 py-1 rounded-lg hover:bg-gray-100">
                                    <SquarePen size={14} /> Update
                                </button>

                                <button className="flex items-center gap-1 text-xs border border-red-200 text-red-500 px-2 py-1 rounded-lg hover:bg-red-50">
                                    <Trash2 size={14} /> Delete
                                </button>

                            </div>

                        </div>

                    </Link>






                </div>


            </div>

        </div>
    );
};

export default AllProjects;