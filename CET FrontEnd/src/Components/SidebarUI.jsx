  ...
  if (!sessionChecked || accountLoading) {
    ...
    return (
      ...
      <aside className="w-72 h-screen bg-white flex flex-col font-jukarta border-r border-slate-100 sticky left-0 top-0 z-[100]">
        ...
        <div
          className={`fixed top-0 transition-all duration-500 ease-in-out ${showNotifications ? "right-0" : "-right-[440px]"
                  ...
                  {item.to && !item.children ? (
                    ...
                    >
                      ...
                      {item.children && (
                        <ChevronRight
                          size={16}
                          className={`transition-transform duration-300 ${openMenu === item.label ? "rotate-90 text-indigo-600" : "text-slate-400"
                            }`}
                        />
                      )}
                    </div>
